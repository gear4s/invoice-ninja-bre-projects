...
class Settings
{
    ...
    {
        $r = $this->powerboard->gatewayRequest('/v1/gateways', (HttpVerb::GET)->value, [], []);

        if ($r->failed()) {
            $r->throw();
        }

        return (new Parse)->encode(Gateway::class . '[]', $r->object()->resource->data);
    ...
    public function getPaymentGatewayConfiguration(int $gateway_type_id): mixed
    {
        ...
        match ($gateway_type_id) {
            GatewayType::CREDIT_CARD => $type = self::GATEWAY_CBA,
    ...
    private function getGatewayByType(string $gateway_type_const): mixed
    {
        ...
        $gateways = (new Parse)->encode(Gateway::class . '[]', $settings->gateways);
