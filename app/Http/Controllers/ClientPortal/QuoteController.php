...
class QuoteController extends Controller
{
    ...
    public function show(ShowQuoteRequest $request, Quote $quote)
    {
        ...
        $data = [
            ...
            'variables' => $variables,
            'requires_signature' => !$signature_accepted && $quote->client->getSetting('require_quote_signature') && $quote->company->account->hasFeature(Account::FEATURE_INVOICE_SETTINGS),
    ...
    public function bulk(ProcessQuotesInBulkRequest $request)
    {
        ...
        if ($request->action == 'approve') {
            ...
            if (auth()->guard('contact')->user()->company->docuninjaActive()) {
                $invitations = QuoteInvitation::with('quote')
