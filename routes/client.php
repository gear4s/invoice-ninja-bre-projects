...
use App\Http\Controllers\ClientPortal\ContactHashLoginController;
...
use App\Http\Controllers\ClientPortal\InvitationController;
use App\Http\Controllers\ClientPortal\NinjaPlanController;
use App\Http\Controllers\ClientPortal\PaymentController;
use App\Http\Controllers\ClientPortal\PaymentMethodController;
use App\Http\Controllers\ClientPortal\PrePaymentController;
use App\Http\Controllers\ClientPortal\ProfileController;
use App\Http\Controllers\ClientPortal\ProjectController;
use App\Http\Controllers\ClientPortal\StatementController;
use App\Http\Controllers\ClientPortal\SubscriptionController;
use App\Http\Controllers\ClientPortal\SubscriptionPlanSwitchController;
use App\Http\Controllers\ClientPortal\SubscriptionPurchaseController;
use App\Http\Controllers\ClientPortal\SwitchCompanyController;
use App\Http\Controllers\ClientPortal\TaskController;
use App\Http\Controllers\ClientPortal\TempRouteController;
use App\Http\Controllers\ClientPortal\UploadController;
...
Route::post('set_password', [InvitationController::class, 'handlePasswordSet'])->name('client.set_password')->middleware('domain_db');

Route::get('tmp_pdf/{hash}', [TempRouteController::class, 'index'])->name('tmp_pdf');

Route::get('client/key_login/{contact_key}', [ContactHashLoginController::class, 'login'])->name('client.contact_login')->middleware(['domain_db', 'contact_key_login']);
Route::get('client/magic_link/{magic_link}', [ContactHashLoginController::class, 'magicLink'])->name('client.contact_magic_link')->middleware(['domain_db', 'contact_key_login']);

Route::get('documents/{document_hash}', [DocumentController::class, 'publicDownload'])->name('documents.public_download')->middleware(['token_auth']);
Route::get('documents/{hash}/hashed', [DocumentController::class, 'hashDownload'])->name('documents.hashed_download');
Route::get('error', [ContactHashLoginController::class, 'errorPage'])->name('client.error');
Route::get('client/payment/{contact_key}/{payment_id}', [InvitationController::class, 'paymentRouter'])->middleware(['domain_db', 'contact_key_login']);
Route::get('client/ninja/{contact_key}/{company_key}', [NinjaPlanController::class, 'index'])->name('client.ninja_contact_login')->middleware(['domain_db']);
...
Route::group(['middleware' => ['auth:contact', 'locale', 'domain_db', 'check_client_existence'], 'prefix' => 'client', 'as' => 'client.'], function () {
    Route::get('dashboard', [DashboardController::class, 'index'])->name('dashboard'); // name = (dashboard. index / create / show / update / destroy / edit

    Route::post('ninja/trial_confirmation', [NinjaPlanController::class, 'trial_confirmation'])->name('trial.response')->middleware(['domain_db']);

    Route::get('plan', [NinjaPlanController::class, 'plan'])->name('plan'); // name = (dashboard. index / create / show / update / destroy / edit
    ...
    Route::get('payments/process', [PaymentController::class, 'catch_process'])->name('payments.catch_process');

    Route::post('payments/credit_response', [PaymentController::class, 'credit_response'])->name('payments.credit_response');

    Route::get('payments', [PaymentController::class, 'index'])->name('payments.index')->middleware('portal_enabled');
    ...
    Route::get('profile/{client_contact}/edit', [ProfileController::class, 'edit'])->name('profile.edit');
    Route::put('profile/{client_contact}/edit', [ProfileController::class, 'update'])->name('profile.update');
    Route::put('profile/{client_contact}/edit_client', [ProfileController::class, 'updateClient'])->name('profile.edit_client');
    Route::put('profile/{client_contact}/localization', [ProfileController::class, 'updateClientLocalization'])->name('profile.edit_localization');

    Route::get('payment_methods/{payment_method}/verification', [PaymentMethodController::class, 'verify'])->name('payment_methods.verification');
    Route::post('payment_methods/{payment_method}/verification', [PaymentMethodController::class, 'processVerification'])->middleware(['throttle:portal']);

    Route::get('payment_methods/confirm', [PaymentMethodController::class, 'store'])->name('payment_methods.confirm');
    ...
    Route::get('client/switch_company/{contact}', SwitchCompanyController::class)->name('switch_company');
    ...
    Route::get('subscriptions/{recurring_invoice}/plan_switch/{target}', [SubscriptionPlanSwitchController::class, 'index'])->name('subscription.plan_switch');
    ...
    Route::get('projects', [ProjectController::class, 'index'])->name('projects.index')->middleware('portal_enabled');
    Route::get('projects/{project}', [ProjectController::class, 'show'])->name('projects.show');

    Route::get('statement', [StatementController::class, 'index'])->name('statement');
    Route::get('statement/raw', [StatementController::class, 'raw'])->name('statement.raw');
...
Route::post('payments/process/response', [PaymentController::class, 'response'])->name('client.payments.response')->middleware(['locale', 'domain_db', 'verify_hash']);
Route::get('payments/process/response', [PaymentController::class, 'response'])->name('client.payments.response.get')->middleware(['locale', 'domain_db', 'verify_hash']);

Route::get('client/subscriptions/{subscription}/purchase', [SubscriptionPurchaseController::class, 'index'])->name('client.subscription.purchase')->middleware('domain_db');
Route::get('client/subscriptions/{subscription}/purchase/v2', [SubscriptionPurchaseController::class, 'upgrade'])->name('client.subscription.upgrade')->middleware('domain_db');
Route::get('client/subscriptions/{subscription}/purchase/v3', [SubscriptionPurchaseController::class, 'v3'])->name('client.subscription.v3')->middleware('domain_db');

Route::group(['middleware' => ['invite_db'], 'prefix' => 'client', 'as' => 'client.'], function () {
    /* Invitation catches */
    Route::get('recurring_invoice/{invitation_key}', [InvitationController::class, 'recurringRouter']);
    Route::get('invoice/{invitation_key}', [InvitationController::class, 'invoiceRouter']);
    Route::get('quote/{invitation_key}', [InvitationController::class, 'quoteRouter']);
    Route::get('credit/{invitation_key}', [InvitationController::class, 'creditRouter']);
    ...
    Route::get('{entity}/{invitation_key}/download', [InvitationController::class, 'routerForDownload'])->middleware('token_auth');
    Route::get('pay/{invitation_key}', [InvitationController::class, 'payInvoice'])->name('pay.invoice');
    ...
    Route::get('unsubscribe/{entity}/{invitation_key}', [InvitationController::class, 'unsubscribe'])->name('unsubscribe');
});

Route::get('.env', function () {})->middleware('throttle:honeypot');
